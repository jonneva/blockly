...
Blockly.Pawn.addReservedWords(
	...
	// page 97
    'assert,defined,#assert,const,break,sizeof,#define,forward,' +
	'case,state,#else,native,continue,tagof,#elseif,new,' +
	'default,#endif,operator,do,#endinput,public,' +
	'else,#error,static,exit,#file,stock,for,#if,' +
	'goto,#include,if,#line,return,#pragma,sleep,#section,' +
	'state,#tryinclude,switch,#undef,while,' +
 ...
 * https://www.dartlang.org/docs/Pawn-up-and-running/ch02.html#operator_table
 */
 ...
 */
Blockly.Pawn.ONE_BASED_INDEXING = true;
...
Blockly.Pawn.init = function(workspace) {
  ...
  if (variables.length) {
    for (var i = 0; i < variables.length; i++) {
      defvars[i] = Blockly.Pawn.variableDB_.getName(variables[i],
...
Blockly.Pawn.getAdjusted = function(block, atId, opt_delta, opt_negate,
  ...
  var defaultAtIndex = Blockly.Pawn.ONE_BASED_INDEXING ? '1' : '0';
  if (delta) {
