...
Blockly.Pawn['controls_if'] = function(block) {
  ...
  var n = 0;
  var argument = Blockly.Pawn.valueToCode(block, 'IF' + n,
      Blockly.Pawn.ORDER_NONE) || 'false';
  var branch = Blockly.Pawn.statementToCode(block, 'DO' + n);
  var code = 'if (' + argument + ') {\n' + branch + '}';
  for (n = 1; n <= block.elseifCount_; n++) {
    argument = Blockly.Pawn.valueToCode(block, 'IF' + n,
      Blockly.Pawn.ORDER_NONE) || 'false';
    branch = Blockly.Pawn.statementToCode(block, 'DO' + n);
    code += ' else if (' + argument + ') {\n' + branch + '}';
  }
  if (block.elseCount_) {
    branch = Blockly.Pawn.statementToCode(block, 'ELSE');
    code += ' else {\n' + branch + '}';
  }
  return code + '\n';
};
